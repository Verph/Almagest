...
public class Moon extends CelestialObject
{
    ...
    public final boolean isParentObserver;
    ...
    public Moon(ClientLevel level, MoonData moon, Player player)
    {
        super(level, player);
        ...
        this.moon = moon;
        this.name = moon.names().getAlphaNameOrDefault();
        ...
        this.color = moon.getColor();
        this.colorOrbit = new Vec3(Config.COMMON.moonOrbitColorRed.get(), Config.COMMON.moonOrbitColorGreen.get(), Config.COMMON.moonOrbitColorBlue.get());
        this.alphaMod = 1.0D;
        ...
        this.wobble = 0.0D; //PlanetDataManager.getAdjustedNodalPrecession(time, orbit, nodalPrecession);
        ...
        this.segments = (int) Math.round(10 + Math.abs(this.semimajorAxis * 0.01D));
        this.isObserver = PlanetDataManager.isObserver(id);
        this.isParentObserver = OBSERVER == parent;
        ...
        this.alpha = 1.0F;
        this.glow = Mth.clamp(Mth.floor(LightTexture.FULL_BRIGHT * (albedo * 5.0D)), Mth.floor(LightTexture.FULL_BRIGHT * 0.5F), LightTexture.FULL_BRIGHT);
        this.player = player;
    }
    ...
    {
        super.tick();
        if (!Config.COMMON.renderMoons.get() || realDiameter < Config.COMMON.moonMinSizeForRender.get()) return;
        ...
        {
            if (Config.COMMON.displayMoonNames.get() && RenderEventHandler.gameTime % 20 == 0)
            {
                if (player != null && RenderEventHandler.isScoping)
                {
                    if (AHelpers.isWithinAngle(RenderEventHandler.lookAngle, pos, Config.COMMON.moonDisplayNameAngleThreshold.get() * RenderEventHandler.fieldOfViewModifier, false))
                    {
        ...
        CelestialObjectHandler.MOON_OBJECTS_CACHE.put(id, this);
        if (!this.isAlive())
        {
            CelestialObjectHandler.MOON_OBJECTS_CACHE.remove(id);
        }
    }
    ...
    public void render(VertexConsumer buffer, Camera camera, float partialTicks)
    {
        ...
        float a = (1.0F - RenderEventHandler.rainLevel) * this.alpha;
        if (!isParentObserver)
        {
            a *= RenderEventHandler.starBrightness;
        }
        if (a > 0.0F && pos != null)
        {
            ...
            {
                if (moon.texture().equals("Unknown"))
                {
